...
- Compose forwards upper- or lower-case HTTP/HTTPS/ALL/NO proxy variables, preferring nonempty uppercase values. Bun's built-in HTTP client honors these variables directly, so proxying works from the container environment alone. The CLI's `--proxy-env` and `--no-proxy-env` flags only affect a Node runtime and have no effect in this image. HTTP proxy support does not imply every SOCKS configuration is supported.
