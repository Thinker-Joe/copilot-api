...
Compose 默认使用持久化命名卷，宿主机只向 127.0.0.1 发布端口。已有 bind mount 的用户必须通过显式 SV 覆盖配置保留原数据源，不能直接切换到新的空命名卷。旧 /root/.local/share/copilot-api 挂载需要迁移到 /data，并准备非 root 用户所需权限。

完整的首次初始化、备份与回滚、目录权限、代理、端口、镜像渠道及 SV 部署方式见 [Docker 部署与迁移](docs/docker.zh-CN.md)。除非明确要删除持久化数据，否则不要执行 docker compose down -v。
